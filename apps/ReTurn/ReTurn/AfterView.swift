...
struct AfterView: View {
    ...
    init(
        ...
        onOpenTodo: @escaping () -> Void = {},
        onAcceptTodo: @escaping (String) -> Void = { _ in },
        onOpenHealth: @escaping () -> Void = {},
        onOpenIdea: @escaping (IdeaCardContent) -> Void = { _ in }
    ) {
                        ...
                        if !ideas.isEmpty {
                            CardGroup("Ideas") {
                                ForEach(ideas.indices, id: \.self) { index in
                                    IdeaCard(content: ideas[index]) {
                                        onOpenIdea(ideas[index])
                                    }
                                }
                            }
                        }
