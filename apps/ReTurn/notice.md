...
- The composer's attachment `Menu` is a single plain-styled control inside the glass content layer. `.buttonStyle(.plain)` is load-bearing twice over: without it the system draws a bordered pressed state around the glyph, and it treats the button as a glass surface to morph into the menu — that morph is what used to make the whole composer disappear when the `Menu` sat inline. Its touch target reaches the 44pt minimum via `padding(hitPadding)` → `contentShape(.rect)` → `padding(-hitPadding)`; never clip it to the glyph bounds (`clipShape`/`contentShape` around a `.title2` symbol yields a ~20pt target).
