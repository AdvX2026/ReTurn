...
- The composer's attachment `Menu` is a transparent hit/anchor layer over the visible plus in the glass content layer. Its label must keep an explicit `composerAccessoryHitSize` frame — do not add `clipShape`/`contentShape` around the glyph, both clip the hit region back under the 44pt minimum. The iOS composer also needs its own `contentShape` + tap-to-focus: the `TextField` only covers one text line, so the surface's vertical padding is otherwise dead.
