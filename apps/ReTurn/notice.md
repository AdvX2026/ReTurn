...
- **Known issue, unsolved:** opening the attachment menu merges the composer's `glassEffect` surface into the menu, so the input field is not visible while the menu is open — with or without the keyboard. Ruled out by testing on iOS 26: `.buttonStyle(.plain)` (removes the pressed box, not the merge) and a UIKit `UIButton` + palette `UIMenu` via `UIViewRepresentable` (tried and reverted — UIKit presentation merges too). A transparent second `Menu` layered outside the glass does avoid it, but the user rejected that two-layer structure. Untried: `GlassEffectContainer` + `glassEffectID`/`glassEffectUnion` to give the plus its own morph identity, and dropping `.interactive()` to see whether the merge is tied to the interactive glass source.
