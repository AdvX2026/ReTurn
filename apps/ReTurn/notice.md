...
- On iOS the composer's attachment button is `AttachmentMenuButton` — a `UIButton` + palette `UIMenu` wrapped in `UIViewRepresentable`, sitting as an ordinary child of the glass content layer. **Do not replace it with a SwiftUI `Menu`.** A SwiftUI `Menu` anywhere inside the composer makes iOS 26 treat the `glassEffect` surface as the menu's presentation source and morph it into the menu, so the input field disappears while the menu is open — this happens with or without the keyboard, and no button/menu style prevents it (`.buttonStyle(.plain)` was tried). UIKit presents the menu without knowing about the SwiftUI glass layer, so only the button lifts. macOS keeps the SwiftUI `Menu`; it has no such morph.
- The attachment button is laid out at `composerAccessoryHitSize` (44pt) and pulled back with `padding(-composerAccessoryHitPadding)`, so it hit-tests the full HIG target while spacing like a 30pt accessory. Never clip it to the glyph bounds — `clipShape`/`contentShape` around a `.title2` symbol yields a ~20pt target, and `clipShape` clips hit testing too.
