...
- **形态**：Orange Pi 3B 家庭服务器（数据中枢，Node 服务 + SQLite）+ macOS 桌面客户端（Tauri 2）+ **iOS 展示端（只读，复用同一 React 代码库打包）**。iPhone 另以**快捷指令**每日定时上传健康数据（睡眠/步数），App 内不写任何 HealthKit 代码。同一局域网内通信，桌面端带离线缓冲。**平台范围就是 macOS + iOS，不做 Windows。**
...
1. **采集（全天自动）**：桌面端常驻采样器每 3~5 分钟记录一次前台应用 + 浏览器标签页，并解析本地 Coding Agent 会话记录，落为节点上报 Pi（离线时本地缓冲，恢复后补传）；iPhone 快捷指令每日定时把昨夜睡眠、当日步数 POST 到 Pi。
...
| F2 | 后台采样器 | 桌面端常驻：每 3~5 分钟采样前台应用 + Chrome/Safari 标签页（AppleScript），并解析本地 Coding Agent 会话记录（`~/.claude/projects/**/*.jsonl`）。采样在上报前由代码聚合为"应用会话"（见 §4.1） |
| F3 | 快速投喂框 | 全局常驻输入：文本直接存；URL 抓取标题+正文摘要；语音按住录音 → 上传 Pi → 云端转写 |
| F4 | Save Today + 存档留言 | 点击 Save → 弹出留言框（语音/文本二选一，可跳过）→ 收尾环境快照 → 触发服务端发酵 |
...
| F11 | iOS 展示端 | 只读薄壳：Continue、角色与属性、状态一览。复用桌面 React 组件出移动子集，Tauri 2 iOS 打包（免费开发者账号真机签名）；打包受阻则降级为 Pi 托管移动 Web 页（同一代码，零改动） |
...
| T0（=F1，必做） | iOS 健康 | 昨夜睡眠时长、当日步数 | iPhone 快捷指令每日定时自动化，家庭 Wi-Fi 下 POST 到 Pi（带固定 token） | 低，"精力"唯一的硬信号 |
...
- 本地一张 `outbox` 表（Tauri 侧 SQLite）：所有写操作（投喂、采样、勾选）先落本地再尝试上报；Pi 不可达时静默积压，恢复后按序补传，靠 `client_uuid` 去重。
- 读操作（Continue、节点流、状态一览）依赖在线；离线时展示最后一次成功拉取的缓存 + "未连接到空间"状态条。
...
```
┌─ Mac A (Tauri2+React) ─────────┐        ┌────── Orange Pi 3B (Debian) ──────┐
│  UI / 采样器 / 录音 / outbox    ├─局域网─▶│  Node 服务 (Fastify)               │──▶ 云端 LLM API
└────────────────────────────────┘  HTTP  │  SQLite / 属性结算 / 发酵编排       │──▶ 云端转写 API
┌─ Mac B（同上，可多台） ─────────┤        │  (LLM/转写 key 只存在 Pi 上)        │
└────────────────────────────────┘        └───────────────▲───────────────────┘
┌─ iPhone ───────────────────────┐                        │
...
| 桌面框架 | Tauri 2 | Rust 层保持薄：本地 outbox SQLite、osascript 采样命令、录音文件落盘 |
...
3. **现场投喂**：打几个字、贴一个链接、录一句语音（现场转写）；切到节点流，展示采样器攒下的应用会话和今天的 Coding Agent 会话（"我们写这个项目的过程本身也被采集了进来"）。
...
- **T+18h**：采样器（采样→会话聚合→上报）+ outbox 离线补传跑通。
...
- **T+40h**：角色系统（5 态整图切换 + 属性面板 + 角色包加载）+ 状态一览；属性公式与状态阈值用真实数据调参。
...
9. **图表库是新增依赖**：状态一览需要轻量图表（候选 Recharts；或热力格+折线手写 SVG 省依赖）。引入前需确认。
