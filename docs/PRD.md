...
- 状态是**文本标签**：展示在 briefing 卡（昨日定格）与 `/api/stats/today`（白天实时，Now 问候文案可引用）；无立绘切换（状态驱动吉祥物**动画选择**为 P1，见 §3）。
- **呈现分层（面向大众的硬约束）**：briefing 卡以**状态标签 + 一句归因**打头（「昨天：疲惫。睡了 5 小时，但代码写了 4 个钟头」），五维数值收在展开/详情层，不把数值条/雷达图直接怼给用户；文案基调是**描述你的一天，不是给你打分**。
