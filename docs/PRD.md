...
>
> v0.5 相对 v0.4 的前端转向：UI 从 Tauri 2 + React 改为 **SwiftUI 原生**（macOS + iOS 单代码库，独立 Xcode 工程）；健康数据改为 **iOS App 内 HealthKit 直读上报为主**，快捷指令定时上传降为备胎（同一端点、按日期幂等，可共存）；随之取消移动 Web 兜底。后端（server / sampler / shared）与 API 合同不受影响。

---

## 1. 产品概述

- **形态**：Orange Pi 3B 家庭服务器（数据中枢，Node 服务 + SQLite）+ macOS 桌面端（**UI App 与常驻采样器双进程分离**，UI 用 SwiftUI）+ **iOS 端（SwiftUI，与桌面同一 Xcode 多平台工程出只读子集）**。健康数据（睡眠/步数）由 **iOS App 内 HealthKit 直读**并上报 Pi；快捷指令每日定时上传作为备胎（同一端点、按日期幂等，可共存）。同一局域网内通信，桌面端带离线缓冲。**平台范围就是 macOS + iOS，不做 Windows。**
...
1. **采集（全天自动）**：桌面端常驻采样器（**独立于 UI 的后台进程**，UI 关闭不影响采集）每 3~5 分钟记录一次前台应用 + 浏览器标签页，并解析本地 Coding Agent 会话记录，落为节点上报 Pi（离线时本地缓冲，恢复后补传）；iOS App 每次进前台经 HealthKit 读昨夜睡眠、当日步数上报 Pi（按日期幂等、重报刷新；快捷指令定时上传兜底）。
...
| 编号 | 功能 | 说明 |
|---|---|---|
| F1 | 多端同步基座 | Pi 上 HTTP API + SQLite；设备注册、离线本地缓冲、恢复连接自动补传；健康数据上报接口（iOS App HealthKit 直读上报为主，快捷指令直连为备胎）；同一空间多设备共享 |
...
| F11 | iOS 端 | 只读展示：Continue、角色与属性、当日时间轴、状态一览，与桌面同一 SwiftUI 多平台代码库出移动子集（免费开发者账号真机签名，受阻则模拟器演示兜底）。另负责 HealthKit 健康同步——进前台即上报，是 iOS 唯一的写操作（见 §3.1 T0） |
...
| T0（=F1，必做） | iOS 健康 | 昨夜睡眠时长、当日步数 | iOS App 内 HealthKit 直读（`sleepAnalysis` 取昨晚窗口合并 asleep 区间；`stepCount` 当日累加），App 进前台即 POST 到 Pi，按日期幂等、重报刷新。备胎：快捷指令每日定时自动化 POST 同一端点（家庭 Wi-Fi + 固定 token，与主路径可共存，谁后到谁刷新）。注意：Apple 无云端 Health API，HealthKit 只能在 iPhone 本机由原生 App 读取——转原生后恰好吃到这个能力 | 低，"精力"唯一的硬信号 |
...
- UI 进程持有自己的**轻量 outbox**（投喂、勾选，量小）：Swift 侧按同一幂等约定实现（本地 JSON 文件队列，重发复用同一 `client_uuid`）；两进程不共享本地库，避免跨进程锁竞争。
...
└────────────────────────────────┘                        │
┌─ iPhone ───────────────────────┐                        │
│  iOS 端 (SwiftUI, 同一 Xcode 工程)├─局域网──────────────┘
│   只读展示 + HealthKit 健康上报  │
│   (快捷指令定时上传作备胎)       │
└────────────────────────────────┘
```

后端全栈 TypeScript（server / sampler / shared 一种语言、一份合同；Pi 上负载只是 SQLite + 转发 LLM 调用，Node 绰绰有余）；前端 Swift/SwiftUI（macOS + iOS 单代码库）：

| 层 | 选型 | 备注 |
|---|---|---|
| 运行时 | Node 22 LTS + tsx（开发与 Pi 上均直接跑 TS，不做构建） | 少一个 build 环节少一类演示事故；不用 Bun（arm64 上多余的风险） |
| Monorepo | pnpm workspaces：`shared` / `server` / `sampler`；`apps/ReturnApp` 为独立 Xcode 多平台工程（同仓、不进 pnpm workspace） | 不上 Turborepo/Nx；Swift 工程与后端契约改动在同一 diff 内可见 |
| 合同 | `shared`：Zod schema（API 请求/响应 + 发酵 JSON + kind 枚举）仍是唯一事实来源；Swift 侧手写 Codable 镜像集中于 `Models.swift` 一处 | 改合同必须同步两份（API 面小，镜像半小时级）；不上 tRPC/代码生成——REST 可 curl 调试，快捷指令备胎也要打裸 HTTP |
...
| 前端（双端） | SwiftUI 多平台工程（macOS 14 / iOS 17 基线），一份代码出 macOS 全功能 + iOS 只读子集 | URLSession async/await 直连 Pi 与采样器；无 WebView、无 JS 运行时 |
| 状态管理 | `@Observable` store + `Task` 循环轮询 `/api/stats/today`（缓存、重试、离线降级展示）；纯 UI 状态用 `@State` | 5 个视图一个 enum 切换即可，不引路由、不上 TCA |
| 录音 | AVAudioRecorder 录 `.m4a`（AAC）→ multipart 上传 `/api/voice` | 比 webview `getUserMedia` 链路稳；UI 轻量 outbox 用本地 JSON 文件队列 |
| 健康 | HealthKit（仅 iOS target，`#if os(iOS)` 圈住）：进前台同步昨夜睡眠 + 当日步数 | 免费个人团队可用该 capability；不做后台推送（BGTask 调试成本高，黑客松不碰） |
| 组件/动效 | 系统组件 + SF Symbols；动效用 SwiftUI 内建（`withAnimation` / PhaseAnimator，结算画面数字弹跳、角色状态揭晓） | 零 UI 依赖；F9 是动效主战场 |
| 图表 | Swift Charts：折线 + 热力格；时间轴（F12）用 Canvas 自绘 | 零第三方图表依赖（见 §9.9 定稿） |
| 网络权限 | Info.plist：`NSAllowsLocalNetworking`（明文 HTTP 打局域网）+ iOS `NSLocalNetworkUsageDescription`（本地网络弹窗）；macOS 不开 App Sandbox（直接分发） | T+2h spike ⑥ 一并验证 |
| 质量工具 | Biome（server/sampler，lint + format 一件套）+ TS strict；Swift 侧用 Xcode 默认诊断 | 不引 SwiftLint，48h 内不值得 |
| 发现 | Pi 起 mDNS（`return.local`）+ 设置页手填 IP 兜底 | 演示用手机热点组网，Pi 配静态 IP；iOS 上 mDNS 偶发不稳，演示直接写死 IP |

明确不用：tRPC、ORM、Next.js/SSR、Electron/Tauri（v0.5 已转原生）、React/Vite 前端栈、TCA、SwiftData/CoreData（UI 无本地库）、SwiftLint、第三方图表库（理由见各行备注与 §9.9）。
...
POST /api/health { date, sleep_minutes, steps }
     iOS App HealthKit 同步为主、快捷指令备胎共用（固定 token 请求头）
     → 落 health_daily 节点（按日期幂等，重报刷新）
GET  /api/nodes?date=
DELETE /api/nodes/:id
...
iOS 端使用全部 GET 接口 + `POST /api/health`（健康上报是 iOS 唯一写操作；投喂、Save、勾选等写接口为 macOS 专属）。
...
- **T+2h**：六个 spike 全部验证通过，跑不通的当场砍或换方案：① Pi 上 Node+SQLite 服务起来（含 better-sqlite3 arm64 安装验证，装不顺换 `node:sqlite`），桌面端局域网可达（mDNS/静态 IP）；② AppleScript 采样前台应用+标签页的授权流程；③ 从 Pi 调通云端 LLM 与转写；④ AVAudioRecorder 录音 → multipart 上传 → Pi 转写全链路；⑤ HealthKit 授权 + 读昨夜睡眠/当日步数并 POST 到 Pi（真机验证）；⑥ SwiftUI 空壳 iOS 真机免费签名装机 + 局域网访问 Pi（本地网络权限弹窗、ATS 明文例外一并验证；受阻当场改判模拟器演示兜底）。
...
- **T+36h**：iOS 端在真机上读到 Pi 的真实数据 + HealthKit 同步跑通（签名受阻则按 spike ⑥ 结论用模拟器演示）。
...
1. **iOS 端签名/权限风险**：免费开发者账号 profile 仅 7 天有效——演示前一天必须重新签名装机（进演示检查清单）；本地网络权限被误拒会导致 App 全盲（设置里可重开）。转原生后移动 Web 兜底不复存在，签名受阻的兜底是模拟器演示，损失"真机装了 App"的说法。T+2h spike ⑥ 验证。
...
9. **图表方案（已定稿，v0.5 更新）**：折线与热力格用系统自带 Swift Charts；时间轴（F12）无现成图表可套，用 Canvas 自绘。仍是零第三方图表依赖。
10. **多设备时序**：两台设备时钟不一致可能导致节点归日错乱。48h 内以 Pi 收到时间为准（`created_at` 服务端盖章），客户端时间仅存 `source_meta` 参考。
11. **健康同步依赖"当天打开过 iOS App"**：HealthKit 走前台同步语义（不做后台推送），哪天没开 App、备胎快捷指令也没触发，当天就没有精力硬信号——精力公式自动回退纯扣分式，不报错不摆烂。演示当天打开一次 iOS App 即可。
