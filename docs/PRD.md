...
> - **角色系统砍掉**，换品牌吉祥物；五维属性收缩为**每日可计算等级**（仍纯代码结算）。
...
晚上        Save：夜间检查点（可附一句留言，可跳过）
             │  系统进入夜间档：降采样、发酵大复盘
             │  产出：briefing 卡 / todo 建议卡 / 健康建议卡 / 连边 / 每日等级
...
| F5 | Save 夜间检查点 + 发酵 | 点 Save（可附一句留言作发酵锚点，可跳过）→ 收尾环境快照 → 夜间发酵：产出次日 briefing 卡（含每日等级 + 昨日简报）、todo 建议卡、健康建议卡、节点标签与连边；等级由纯代码结算。幂等：当日已 Save 直接返回 |
...
- Future 侧时间线 / 计划轴。
- 角色系统、五维属性面板、结算画面、角色包（v0.5 遗产，v0.6 砍除；吉祥物替代，见 §4）。
- 深度采集（屏幕截图/OCR/键鼠活跃度）。
...
- 实现：静态图 + SwiftUI 轻动效（淡入、弹跳）；表情图 1~3 张封顶，美术量远小于 v0.5 的角色包。**不由数值驱动、无状态机、无换装体系。**
- v0.5 的五维属性面板、状态立绘、结算画面、角色包规范全部移除（P2）。

### 4.2 每日可计算等级 —— 纯代码结算，LLM 不打分

- 保留 v0.5「可解释、可复现」原则：当日信号加权合成 0~100 综合分，映射到等级档（S/A/B/C/D），随夜间发酵写入 `days.stats_json`，展示在次日 briefing 卡上。
- 信号沿用 v0.5 五维的可算项收缩为单值：主动投喂/对话量、应用会话专注度（HHI）、todo 完成率与 Coding Agent 会话时长、跨日连边数、昨夜睡眠/步数。系数为初值，演示前用全队真实数据调参。
...
days(id, date, saved_at, save_note_node_id, summary, stats_json /* 等级结算 */)
...
│ 采样器 (Node, launchd 常驻)     │  HTTP  │  发酵 / 等级结算                    │──▶ 云端视觉 API(可选)
...
GET  /api/days?range=30             → 等级/streak 历史（周/月视图数据源）
PATCH /api/todos/:id                → 勾选（同时落 todo_check 节点）
GET  /api/stats/today               → 当日实时等级 + 采集状态（Now 问候可用）
...
- **夜间发酵（Save 触发）**：1~2 次主模型调用，输入「当日节点 + 对话 + 高权重资料 + 留言锚点 + 近几日摘要」，输出结构化 JSON：`{ summary, briefing, review_points[], todos[], health_advice, ideas[], node_tags{}, edges[] }` → Zod 校验 → 落 cards/edges/todos → 纯代码结算等级。采样节点只作「环境上下文」。
- 所有 LLM 调用带超时 + 一次重试 + Zod 校验；发酵失败降级回放上一次已落库结果，不阻塞 Save 主流程。

## 7. 演示脚本（3 分钟）

1. **掏出 Orange Pi 实物**：第二大脑物理地住在家里，Mac 和 iPhone 连的是同一块板子。
2. 打开 App → **Now**：吉祥物问候 + 昨日 briefing 卡（提前真实 Save 过一天，含每日等级与 streak）；上刷 **Before**：昨日卡片 → 时间线回溯，「几点在写代码、几点在开会，它都知道」。
...
2. **server 扩展**：三张新表迁移；chat 分诊 + 提问 RAG + resume；save 语义扩展（卡片产出、等级结算收缩）；timeline 范围化；节奏模式下发。
3. **SwiftUI 主线**：双向流主视图（Before/Now/Future）+ Input + sidebar + 时间线回溯（P0-A 门面，最大单块工作量）。
4. **P0-B 工作流**：灵感、检索定位、Task（纪要文本）、健康建议卡；视觉 API 视时间。
5. **收尾**：sampler 节奏模式联调、iOS 真机、等级公式调参、全队真实使用攒演示数据、排练、录备用视频。周/月视图等 P1 项有余力再进。
...
13. **等级公式未调参**：收缩后的综合分系数全是拍的初值，演示前用全队真实数据调；briefing 展示相对变化感，绝对数值不必精确。
14. **双进程编排**：采样器没起来会静默丢采集。缓解：UI 常显采集状态条（经 localhost 健康检查）；launchd `KeepAlive` 兜底；演示前检查清单含「采样器已运行」。
15. **健康同步依赖「当天打开过 iOS App」**：哪天没开 App、备胎快捷指令也没触发，当天就没有精力硬信号——等级公式该项自动缺省，不报错。演示当天打开一次 iOS App 即可。
