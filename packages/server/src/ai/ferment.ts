...
- briefing: one lead line for next-day card (state + short attribution), Chinese OK.
...
- health_advice: optional short tip from sleep/energy context; null if none.
- ideas: 0–3 auto-extracted idea snippets from the day (not user-submitted).
...
export function buildFermentContext(input: {
  ...
  const activeKinds = new Set(["text", "url", "voice", "save_note", "idea"]);
  return {
    date: input.date,
