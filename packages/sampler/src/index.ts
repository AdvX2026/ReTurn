...
async function sampleOnce(opts?: { asSnapshot?: boolean }): Promise<{
  ...
  console.log(
    `[sampler] sample app=${snapshot.app?.name ?? "-"} tabs=${envStats.tabs ?? snapshot.tabs.length} agents=${agentStats.intervals ?? 0} commits=${gitStats.commits ?? 0} mails=${gmailStats.emails ?? 0} emitted=${enqueued} flushed=${flush.flushed} outbox=${flush.remaining} pi=${flush.online}`,
  );
